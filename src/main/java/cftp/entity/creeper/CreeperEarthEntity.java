...
public class CreeperEarthEntity extends CreeperElementalEntity {
    ...
    protected float power = CFTPData.creeperEarth.power;
    ...
    public static DefaultAttributeContainer.Builder createAttributes() {
        return MobEntity.createMobAttributes()
                .add(EntityAttributes.MAX_HEALTH, CFTPData.creeperEarth.health)
                .add(EntityAttributes.MOVEMENT_SPEED, CFTPData.creeperEarth.movementSpeed)
                .add(EntityAttributes.ATTACK_DAMAGE, 0)
                .add(EntityAttributes.FOLLOW_RANGE, CFTPData.creeperEarth.followRange);
    }
                    ...
                    for (int x = 0; x < iDiameter; ++x) {
                        for (int z = 0; z < iDiameter; ++z) {
                            if (Shapes.isSphere(x, y, z, radius)) {
                                ...
                                if (resistance < 100) {
                                    CreeperMath.onGeneralReplace(serverWorld, dummyExplosion, state, CreeperMath.EARTH_BLOCKS[index], blockPos, (itemStack, pos) -> {
